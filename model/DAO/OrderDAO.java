...
public class OrderDAO {
	...
	public List<OrderVO> selectAll() throws SQLException{
		...
		list = sqlSession.selectList("mapper.member.selectAllOrderList");
	...
	public OrderVO selectOne(String orderCd) throws SQLException {
		OrderVO order = null;
		order = (OrderVO)sqlSession.selectOne("mapper.member.selectOrderByOrderCd", orderCd);
		return order;
	}

	public boolean insertOne(OrderVO order) throws SQLException {
		boolean flag = false;
		int affectedCount = sqlSession.insert("mapper.member.insertOrder", order);
		if(affectedCount > 0) {
			flag = true;
		}
	...
	public boolean updateOne(int orderSeq) throws SQLException {
		boolean flag = false;
		int affectedCount = sqlSession.update("mapper.member.updateOrderState", orderSeq);
		if(affectedCount > 0) {
			flag = true;
		}
		return flag;
	...
	public boolean deleteOne(String orderCd) throws SQLException {
		boolean flag = false;
		int affectedCount = sqlSession.delete("mapper.member.deleteOrder", orderCd);
		if(affectedCount > 0) {
			flag = true;
		}
		return flag;
